...
public class SimulationService {
    ...
    public void simulate(String sessionId) {
        ...
        // computeIfAbsent guarantees a single subscription per session.
    ...
    private Mono<Void> buildSimulation(String sessionId) {
        return gameEngineClient.ensureGame(sessionId)
                ...
                .doFinally(signal -> {
                    ...
                    // Remove asynchronously so this can never run inside computeIfAbsent's
                    // internal lock, regardless of the chain's first operator is sync or async
                    Mono.fromRunnable(() -> activeSimulations.remove(sessionId))
