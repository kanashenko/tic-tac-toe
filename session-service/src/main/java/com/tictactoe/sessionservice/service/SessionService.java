...
public class SessionService {
    ...
    /**
     * Verifies the session exists, then kicks off its automated play-out (run
     * in the background by {@link SimulationService}). Fails with
     * {@link com.tictactoe.sessionservice.exception.SessionNotFoundException} —
     * surfaced as a 404 — for an unknown session
     */
    public Mono<Void> startSimulation(String sessionId) {
        return findSession(sessionId)
                .doOnNext(_ -> simulationService.simulate(sessionId))
                .then();
    }
