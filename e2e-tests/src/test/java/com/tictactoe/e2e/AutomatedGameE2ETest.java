...
class AutomatedGameE2ETest {
    ...
    private static final WebSocketClient WS_CLIENT = new ReactorNettyWebSocketClient();
    ...
    void testAutomatedGameFlow() {
        ...
        Disposable wsSubscription = WS_CLIENT.execute(wsUri, wsSession -> {
                    wsConnectedSink.tryEmitEmpty(); // Signal that the channel is open
                    return wsSession.receive()
                            .map(WebSocketMessage::getPayloadAsText)
                            .map(payload -> OBJECT_MAPPER.readValue(payload, SessionResponse.class))
