...
/**
 * An immutable 3x3 Tic Tac Toe board, stored as a flat, row-major list of 9
 * cells so that win-line checks are simple index lookups rather than 2D
 * traversals.
 *
 * <p>Every mutating-looking operation ({@link #place(int, int, char)})
 * returns a new {@code Board} instead of modifying this one, which is what
 * lets {@link Game} stay an immutable record itself — a move produces a new
 * {@code Game} rather than mutating shared state.
 *
 * @param cells the 9 board cells in row-major order (index {@code row * 3 + col})
 */
public record Board(List<Character> cells) {

    /** Symbol used for a cell that has not been played yet. */
    public static final char EMPTY = '-';
    private static final int SIZE = 3;

    /** Every row, column and diagonal, expressed as flat-array indices. */
    ...
    /**
     * Returns a new board with {@code symbol} placed at ({@code row}, {@code col}).
     * Does not validate that the cell is empty — callers (see {@link Game#applyMove})
     * are expected to check {@link #isOccupied(int, int)} first.
     */
    ...
    /**
     * Returns the symbol that occupies a completed row, column or diagonal, if any.
     * Empty is by definition never a winning line.
     */
